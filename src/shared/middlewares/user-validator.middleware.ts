...
import { createUserValidationSchema, logUserValidationSchema } from "../validators/user.joi.validator.js";
...
export const logUserValidator = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.body) {
            res.status(httpStatusCodes.BAD_REQUEST).send({message: 'Missing request body'})
        }

        await logUserValidationSchema.validateAsync(req.body)
